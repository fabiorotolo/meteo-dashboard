...
let currentSensor = 'INT';  // INT o EXT
let allIntData = [];
let allExtData = [];
...
async function loadAndRender() {
  ...
  try {
    ...
    if (allIntData.length === 0) {
      allIntData = await fetchChannelFeeds(INTERNAL_CHANNEL_ID, INTERNAL_READ_KEY, 8000);
    }
    if (allExtData.length === 0) {
      allExtData = await fetchChannelFeeds(EXTERNAL_CHANNEL_ID, EXTERNAL_READ_KEY, 8000);
    }

    // Aggiorna stato in alto
    updateTopStats();
    
    // Rendering grafici in base al sensore selezionato
    renderCharts();
    ...
    statusEl.textContent = `Confronto ${currentDays} giorni - Sensori ${currentSensor}`;
  } catch (err) {
    console.error(err);
...
function updateTopStats() {
  // Ultimo valore interno
  if (allIntData.length > 0) {
    const last = allIntData[allIntData.length - 1];
    const temp = parseFloat(last.raw["field" + INTERNAL_FIELDS.temp]);
    const press = parseFloat(last.raw["field" + INTERNAL_FIELDS.press]);
    const cpu = parseFloat(last.raw["field" + INTERNAL_FIELDS.cpu]);
    
    if (Number.isFinite(temp)) {
      document.getElementById("stat-temp-int").textContent = temp.toFixed(2) + " °C";
    }
  ...
  // Ultimo valore esterno
  if (allExtData.length > 0) {
    const last = allExtData[allExtData.length - 1];
    const temp = parseFloat(last.raw["field" + EXTERNAL_FIELDS.temp]);
    const hum = parseFloat(last.raw["field" + EXTERNAL_FIELDS.hum]);
  ...
  const totalPoints = allIntData.length + allExtData.length;
  document.getElementById("stat-total-points").textContent = totalPoints;
}

function renderCharts() {
  // PRESSIONE (sempre INT)
  ...
  // TEMPERATURA e UMIDITÀ in base al sensore
  if (currentSensor === 'INT') {
    ...
    document.getElementById("temp-title").textContent = "Temperatura INT - Confronto";
    ...
    document.getElementById("hum-title").textContent = "Umidità INT - Confronto";
    ...
    document.getElementById("temp-subtitle").textContent = 
      `Min: ${tempMin.toFixed(1)} | Max: ${tempMax.toFixed(1)} °C`;
    document.getElementById("temp-title").textContent = "Temperatura EXT - Confronto";
    ...
    document.getElementById("hum-subtitle").textContent = 
      `Min: ${humMin.toFixed(1)} | Max: ${humMax.toFixed(1)} %`;
    document.getElementById("hum-title").textContent = "Umidità EXT - Confronto";
  }
}
...
function buildForecastFeatures() {
  if (!allIntData.length) return null;

  const recentPress = filterByRange(allIntData, MAX_WINDOW_HOURS);
  ...
  const recentExt = filterByRange(allExtData, MAX_WINDOW_HOURS);
